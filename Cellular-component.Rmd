...
cc <- read.csv ('./ccncb.csv', sep=";")
cc <- cc[order(cc$GO),]
cc1 <- cc[,2:13]
cc2 <- cc[,14:25]

ccmat1 <- data.matrix(cc1)
ccmat2 <- data.matrix(cc2)
row.names(ccmat1) <- cc$GO
ccmat1[ccmat1 == 1] <- NA
F_m2 <-(apply(ccmat2, 2, function(x){log2(x/median(x, na.rm = T))}))

dimnames(F_m2)[1] <- list(cc$GO)
...
heatmap.2(scale(ccmat1), main = 'GO: Cellular Component',
          ...
          cexRow=0.6, cexCol = 1,      # size of row / column labels
